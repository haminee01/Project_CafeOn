...
import { getChatHistory, patchReadStatus, sendChatMessage } from "@/api/chat";
import { ChatHistoryMessage } from "@/api/chat";

export default function MyPageChatPage() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      console.log("마이페이지 채팅 - 히스토리 로드 시작:", { roomId, jump });
  ...
  const connectWebSocket = useCallback(() => {
    ...
    console.log("마이페이지 채팅 - WebSocket 연결 시작:", { roomId, wsUrl });
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-white">
      {/* 채팅 헤더 */}
      <div className="p-4 border-b border-[#CDCDCD] bg-white">
      ...
      {/* 메시지 입력 영역 */}
      <div className="p-4 border-t border-[#CDCDCD] bg-white">
        {error && (
